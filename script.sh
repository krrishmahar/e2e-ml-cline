...
main() {
    ...
    # 8. Generate docs (if installed)
    if [ "$coderabbit_installed" = true ]; then
        log "INFO" "📚 Generating docs via Coderabbit..."
        if [ "$DRY_RUN" = true ]; then
            log "INFO" "Dry run mode - would run: coderabbit --plain clinerules-bank/prompts/gh_docs.prompt.md"
        else
            if [ "$VERBOSE" = true ]; then
                coderabbit --plain clinerules-bank/prompts/gh_docs.prompt.md
            else
                coderabbit --plain clinerules-bank/prompts/gh_docs.prompt.md > /dev/null 2>&1
            fi
            if [ $? -ne 0 ]; then
                handle_error "Documentation generation" $?
                if [ "$SKIP_MISSING_TOOLS" = false ]; then
                    return 1
                fi
            fi
        ...
        log "INFO" "Skipping documentation generation (tool not installed)"
    fi

    # 9. Update Changelog (if installed)
    if [ "$coderabbit_installed" = true ]; then
        log "INFO" "📝 Updating Changelog..."
        if [ "$DRY_RUN" = true ]; then
            log "INFO" "Dry run mode - would run: coderabbit --plain clinerules-bank/prompts/gh_changelog.mmd"
        else
            if [ "$VERBOSE" = true ]; then
                coderabbit --plain clinerules-bank/prompts/gh_changelog.mmd
            else
                coderabbit --plain clinerules-bank/prompts/gh_changelog.mmd > /dev/null 2>&1
            fi
            if [ $? -ne 0 ]; then
                handle_error "Changelog update" $?
                if [ "$SKIP_MISSING_TOOLS" = false ]; then
                    return 1
                fi
            fi
        ...
        log "INFO" "Skipping Changelog update (tool not installed)"
    fi
